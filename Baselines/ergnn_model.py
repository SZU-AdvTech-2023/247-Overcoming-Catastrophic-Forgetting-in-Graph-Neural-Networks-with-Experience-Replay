...
class NET(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.lamd = float(args.ergnn_args['lamd'])
    ...
    def observe_task_IL(self, args, g, features, labels, t, train_ids, ids_per_cls, dataset):
        """
                        ...
                        :param features: Node features of the current task.
                        :param labels: Labels of the nodes in the current task.
                        :param t: Index of the current task.第几次任务
                        :param train_ids: The indices of the nodes participating in the training.
                        :param ids_per_cls: Indices of the nodes in each class.
                        :param dataset: The entire dataset.
        ...
        if t!=0:
            for oldt in range(t):
                ...
                save_path = f'./para/model_task_IL_batch_para_{t - 1}.pth'
                # 设置旧的参数为不计算梯度
                last_para = torch.load(save_path)
                last_first_layer_params = last_para['gat_layers.0.linear.weight'].detach().requires_grad_(False)
                last_second_layer_params = last_para['gat_layers.1.linear.weight'].detach().requires_grad_(False)

                # now_para  = self.net.state_dict()

                now_first_layer_params = self.net.gat_layers[0].linear.weight
                now_second_layer_params = self.net.gat_layers[1].linear.weight

                # 计算第一层参数的 L2 范数差异
                l2_norm_diff_first_layer = torch.norm(now_first_layer_params - last_first_layer_params, p=2).to(
                    device='cuda:{}'.format(args.gpu))

                # 计算第二层前 n 行参数的 L2 范数差异
                n = t * args.n_cls_per_task  # 请替换为你想要比较的行数
                l2_norm_diff_second_layer_subset = torch.norm(
                    now_second_layer_params[:n] - last_second_layer_params[:n], p=2).to(
                    device='cuda:{}'.format(args.gpu))

                # 现在你可以将这两者加起来，形成整体的差距
                total_l2_norm_diff = (l2_norm_diff_first_layer + l2_norm_diff_second_layer_subset).to(
                    device='cuda:{}'.format(args.gpu))
                # 将 L2 范数差异的梯度加入到反向传播中
                # total_l2_norm_diff.backward(retain_graph=True)
                loss = beta * loss + (1 - beta) * loss_aux + self.lamd * total_l2_norm_diff/1000.

                '''print(f"loss={loss}", f"total_l2_norm_diff={self.lamd * total_l2_norm_diff * 100}",
                      f"last_loss={loss - self.lamd * total_l2_norm_diff * 100}")'''
        ...
        for input_nodes, output_nodes, blocks in dataloader:
            ...
            if t != 0:
                for oldt in range(t):
                    ...
                    # 计算第一层参数的 L2 范数差异
                    l2_norm_diff_first_layer = torch.norm(now_first_layer_params - last_first_layer_params, p=2).to(device='cuda:{}'.format(args.gpu))

                    # 计算第二层前 n 行参数的 L2 范数差异
                    n = t * args.n_cls_per_task  # 请替换为你想要比较的行数
                    l2_norm_diff_second_layer_subset = torch.norm(
                        now_second_layer_params[:n] - last_second_layer_params[:n], p=2).to(device='cuda:{}'.format(args.gpu))

                    # 现在你可以将这两者加起来，形成整体的差距
                    total_l2_norm_diff = (l2_norm_diff_first_layer + l2_norm_diff_second_layer_subset).to(device='cuda:{}'.format(args.gpu))
                    # 将 L2 范数差异的梯度加入到反向传播中
                    #total_l2_norm_diff.backward(retain_graph=True)
                    loss = beta * loss + (1 - beta)* loss_aux+ self.lamd*total_l2_norm_diff/10


                    #print(f"loss={loss}",f"total_l2_norm_diff={self.lamd*total_l2_norm_diff*100}",f"last_loss={loss-self.lamd*total_l2_norm_diff*100}")
    ...
    def observe_class_IL_batch(self, args, g, dataloader, features, labels, t, train_ids, ids_per_cls, dataset):
        ...
        for input_nodes, output_nodes, blocks in dataloader:
            ...
            if t != 0:
                ...
                save_path = f'./para/model_classIL_batch_para_{t - 1}.pth'
                # 获取前一个神经网络的参数
                last_para = torch.load(save_path)
                last_first_layer_params = last_para['gat_layers.0.linear.weight']
                last_second_layer_params = last_para['gat_layers.1.linear.weight']

                now_para = self.net.state_dict()

                # 获取第一层和第二层参数的张量

                now_first_layer_params = now_para['gat_layers.0.linear.weight']


                now_second_layer_params = now_para['gat_layers.1.linear.weight']
                ...
                l2_norm_diff_second_layer_subset = torch.norm(
                    now_second_layer_params[:n] - last_second_layer_params[:n], p=2).to(device='cuda:{}'.format(args.gpu))
                # l2_norm_diff_second_layer_subset*=(args.n_cls//args.n_cls_per_task)/t
                # 现在你可以将这两者加起来，形成整体的差距
                total_l2_norm_diff = (l2_norm_diff_first_layer + l2_norm_diff_second_layer_subset).to(device='cuda:{}'.format(args.gpu))
                # 将 L2 范数差异的梯度加入到反向传播中
                #total_l2_norm_diff.backward(retain_graph=True)

                loss = beta * loss + (1 - beta) * loss_aux+ self.lamd/10*total_l2_norm_diff
            loss.backward()
            self.opt.step()
